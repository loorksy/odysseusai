...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # Native CUDA mode intentionally skips this branch so it exercises the
    # managed source build with GGML_CUDA=ON.
    ...
    runner_lines.append('    _odysseus_native_cuda_requested() { case "${ODYSSEUS_LLAMA_CPP_CUDA:-}" in ON|on|1|true|TRUE|yes|YES) return 0 ;; *) return 1 ;; esac; }')
    ...
    runner_lines.append('    _odysseus_prebuilt_backend=""')
    runner_lines.append('    if _odysseus_native_cuda_requested; then')
    runner_lines.append('      echo "[odysseus] Native CUDA llama.cpp build requested — skipping prebuilt release asset."')
    runner_lines.append('    elif command -v curl >/dev/null 2>&1 && [ "$_odysseus_arch" = "x86_64" ]; then')
    ...
    runner_lines.append('        _odysseus_prebuilt_backend="cuda"')
    ...
    runner_lines.append('        _odysseus_prebuilt_backend="vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _odysseus_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('        _odysseus_prebuilt_backend="cpu"')
    runner_lines.append('      fi')
    ...
    runner_lines.append('      mkdir -p ~/bin "$ODYSSEUS_LLAMA_CPP_PREBUILT_DIR" && cd "$ODYSSEUS_LLAMA_CPP_PREBUILT_DIR"')
    ...
    runner_lines.append('          mkdir -p "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR" && echo "export LD_LIBRARY_PATH=\\"$_odysseus_libdir:\\${LD_LIBRARY_PATH:-}\\"" > "$ODYSSEUS_LLAMA_CPP_RUNTIME_ENV"')
    runner_lines.append('          printf "%s\\n" "$_odysseus_prebuilt_backend" > "$ODYSSEUS_LLAMA_CPP_BACKEND_FILE"')
    ...
    runner_lines.append('    elif [ -z "$_odysseus_prebuilt_url" ] && ! _odysseus_native_cuda_requested; then')
    ...
    runner_lines.append('      for _cudir in ~/.local/lib/python*/site-packages/nvidia/cu13 ~/.local/lib/python*/site-packages/nvidia/cu12 ~/.local/lib/python*/site-packages/nvidia/cuda_nvcc /usr/local/lib/python*/site-packages/nvidia/cu13 /usr/local/lib/python*/site-packages/nvidia/cu12 /usr/local/lib/python*/site-packages/nvidia/cuda_nvcc /usr/local/lib/python*/dist-packages/nvidia/cu13 /usr/local/lib/python*/dist-packages/nvidia/cu12 /usr/local/lib/python*/dist-packages/nvidia/cuda_nvcc /usr/lib/python*/site-packages/nvidia/cu13 /usr/lib/python*/site-packages/nvidia/cu12 /usr/lib/python*/site-packages/nvidia/cuda_nvcc /usr/lib/python*/dist-packages/nvidia/cu13 /usr/lib/python*/dist-packages/nvidia/cu12 /usr/lib/python*/dist-packages/nvidia/cuda_nvcc; do')
    ...
    runner_lines.append('          [ -d "$_culib" ] && export LD_LIBRARY_PATH="$_culib:${LD_LIBRARY_PATH:-}" && export CMAKE_LIBRARY_PATH="$_culib:${CMAKE_LIBRARY_PATH:-}"')
    runner_lines.append('        done')
    ...
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release -DGGML_HIP=ON && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server && mkdir -p "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR" && printf "hip\\n" > "$ODYSSEUS_LLAMA_CPP_BACKEND_FILE"')
    ...
    runner_lines.append('      _odysseus_cuda_cudart=""')
    runner_lines.append('      _odysseus_cuda_cublas=""')
    runner_lines.append('      _odysseus_cuda_cublaslt=""')
    runner_lines.append('      _odysseus_remember_cuda_lib() {')
    runner_lines.append('        local _var="$1" _name="$2" _dir _lib')
    runner_lines.append('        shift 2')
    runner_lines.append('        for _dir in "$@"; do')
    runner_lines.append('          for _lib in "$_dir"/lib"${_name}".so*; do')
    runner_lines.append('            [ -e "$_lib" ] || continue')
    runner_lines.append('            printf -v "$_var" "%s" "$_lib"')
    runner_lines.append('            export LD_LIBRARY_PATH="$_dir:${LD_LIBRARY_PATH:-}"')
    runner_lines.append('            export CMAKE_LIBRARY_PATH="$_dir:${CMAKE_LIBRARY_PATH:-}"')
    ...
    runner_lines.append('        local _nvroot="$(dirname "$_cuh")"')
    runner_lines.append('        _odysseus_remember_cuda_lib _odysseus_cuda_cudart cudart "$_cuh/lib64" "$_cuh/lib" /usr/local/cuda/lib64 /usr/local/cuda/lib "$_nvroot/cuda_runtime/lib" ~/.local/lib/python*/site-packages/nvidia/cuda_runtime/lib /usr/local/lib/python*/site-packages/nvidia/cuda_runtime/lib /usr/local/lib/python*/dist-packages/nvidia/cuda_runtime/lib /usr/lib/python*/site-packages/nvidia/cuda_runtime/lib /usr/lib/python*/dist-packages/nvidia/cuda_runtime/lib && return 0')
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    ...
    runner_lines.append('        _odysseus_cuda_cmake_args=()')
    runner_lines.append('        _odysseus_cuda_lib_dirs=("$CUDA_HOME/lib64" "$CUDA_HOME/lib" /usr/local/cuda/lib64 /usr/local/cuda/lib "$(dirname "$CUDA_HOME")/cuda_runtime/lib" "$(dirname "$CUDA_HOME")/cublas/lib" ~/.local/lib/python*/site-packages/nvidia/cuda_runtime/lib ~/.local/lib/python*/site-packages/nvidia/cublas/lib /usr/local/lib/python*/site-packages/nvidia/cuda_runtime/lib /usr/local/lib/python*/site-packages/nvidia/cublas/lib /usr/local/lib/python*/dist-packages/nvidia/cuda_runtime/lib /usr/local/lib/python*/dist-packages/nvidia/cublas/lib /usr/lib/python*/site-packages/nvidia/cuda_runtime/lib /usr/lib/python*/site-packages/nvidia/cublas/lib /usr/lib/python*/dist-packages/nvidia/cuda_runtime/lib /usr/lib/python*/dist-packages/nvidia/cublas/lib)')
    runner_lines.append('        [ -z "$_odysseus_cuda_cublas" ] && _odysseus_remember_cuda_lib _odysseus_cuda_cublas cublas "${_odysseus_cuda_lib_dirs[@]}" || true')
    runner_lines.append('        [ -z "$_odysseus_cuda_cublaslt" ] && _odysseus_remember_cuda_lib _odysseus_cuda_cublaslt cublasLt "${_odysseus_cuda_lib_dirs[@]}" || true')
    runner_lines.append('        if [ -n "$_odysseus_cuda_cudart" ]; then')
    runner_lines.append('          _odysseus_cuda_cmake_args+=("-DCUDA_CUDART=$_odysseus_cuda_cudart" "-DCUDA_cudart_LIBRARY=$_odysseus_cuda_cudart")')
    runner_lines.append('        fi')
    runner_lines.append('        [ -n "$_odysseus_cuda_cublas" ] && _odysseus_cuda_cmake_args+=("-DCUDA_cublas_LIBRARY=$_odysseus_cuda_cublas")')
    runner_lines.append('        [ -n "$_odysseus_cuda_cublaslt" ] && _odysseus_cuda_cmake_args+=("-DCUDA_cublasLt_LIBRARY=$_odysseus_cuda_cublaslt")')
    runner_lines.append('        case "$_odysseus_cuda_cudart" in')
    runner_lines.append('          *site-packages/nvidia/*|*dist-packages/nvidia/*)')
    runner_lines.append('            _odysseus_cuda_cmake_args+=("-DCMAKE_CUDA_FLAGS=${CMAKE_CUDA_FLAGS:-} -DCCCL_DISABLE_CTK_COMPATIBILITY_CHECK")')
    runner_lines.append('            ;;')
    runner_lines.append('        esac')
    runner_lines.append('        cmake -B build -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON "${_odysseus_cuda_cmake_args[@]}" && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server && mkdir -p "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR" && printf "cuda\\n" > "$ODYSSEUS_LLAMA_CPP_BACKEND_FILE" && _odysseus_persist_llama_runtime_env')
    runner_lines.append('      else')
    ...
    runner_lines.append('        cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server && mkdir -p "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR" && printf "cpu\\n" > "$ODYSSEUS_LLAMA_CPP_BACKEND_FILE"')
    runner_lines.append('      fi')
    ...
    runner_lines.append('      cmake -B build-vulkan -DCMAKE_BUILD_TYPE=Release -DGGML_VULKAN=ON && cmake --build build-vulkan -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build-vulkan/bin/llama-server ~/bin/llama-server && mkdir -p "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR" && printf "vulkan\\n" > "$ODYSSEUS_LLAMA_CPP_BACKEND_FILE"')
    runner_lines.append('    else')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    return (
        'ODYSSEUS_LLAMA_CPP_MANAGED_DIR="${ODYSSEUS_LLAMA_CPP_MANAGED_DIR:-$HOME/llama.cpp/.odysseus}" && '
        'mkdir -p "$HOME/bin" && '
        f'{update_cmd}'
        'rm -f "$HOME/bin/llama-server" && '
        'rm -f "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR/backend" "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR/runtime.env" '
        '"$HOME/.config/odysseus-llama-cpp-backend" "$HOME/.config/odysseus-llama-cpp-env" && '
        'rm -rf "$HOME/llama.cpp/build" "$HOME/llama.cpp/build-vulkan" "$ODYSSEUS_LLAMA_CPP_MANAGED_DIR/prebuilt" && '
