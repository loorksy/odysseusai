...
def add_user_message(sess, chat_handler, preprocessed: PreprocessedMessage, incognito: bool = False):
    ...
    chat_handler.session_manager.add_message(sess.id, ChatMessage("user", preprocessed.user_content, metadata=user_meta))
    if not incognito:
...
def save_assistant_response(
    ...
    *,
    ...
    session_manager.add_message(sess.id, ChatMessage("assistant", _content, metadata=md))
