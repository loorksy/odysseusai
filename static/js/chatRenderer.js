...
// Model pricing table — loaded from shared JSON, with inline fallback
let MODEL_INFO = {};
let _pricingLoaded = false;

async function _loadPricingData() {
  if (_pricingLoaded) return;
  try {
    const res = await fetch('/static/data/model_pricing.json');
    if (res.ok) MODEL_INFO = await res.json();
  } catch (_e) { /* fallback to empty — cost will show as unknown */ }
  _pricingLoaded = true;
}
_loadPricingData();
