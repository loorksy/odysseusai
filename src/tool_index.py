...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "create_document": "Create a new document in the editor panel. For code, articles, text content longer than 15 lines, unless an already-open document/email draft is the obvious target. If an email compose draft is open, edit that draft instead of creating another document. When the user asks for a CODE FILE / SCRIPT (e.g. 'a python script'), write the RAW code as the content — do NOT wrap it in a markdown '# heading' or ``` fences — and set language to that code's language so it's a real, runnable file.",
    "edit_document": "Preferred tool for editing an existing document — targeted find-and-replace. Use for any small change: add a function, fix a bug, tweak a section, rename things. When the doc is a code file, FIND/REPLACE raw code (no ``` fences).",
    "update_document": "Replace the entire active document content. ONLY for full rewrites (>50% changed). Do not use for small edits — use edit_document instead. For a CODE FILE / SCRIPT request, write RAW code as the content (no markdown '# heading' or ``` fences) — the document's language follows the content.",
